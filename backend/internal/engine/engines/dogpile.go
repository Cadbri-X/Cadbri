...
type DogpileEngine struct {
	engine.BaseEngine
}

func NewDogpileEngine() *DogpileEngine {
	return &DogpileEngine{
...
func (d *DogpileEngine) Search(ctx context.Context, client *network.Client, query types.SearchQuery, container *results.ResultContainer) error {
	...
	searchURL := fmt.Sprintf("https://www.dogpile.com/serp?q=%s", url.QueryEscape(query.Query))
	...
	// Fallback to Bing if Dogpile rate limits or is blocked
	if len(items) == 0 {
